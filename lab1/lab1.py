...
def computeMSE(prev, curr):
    ...
    for x in range (0, h) :
        for y in range (0, w) :
            mse += math.pow(int(curr[x,y]) - int(prev[x,y]),2)
...
def computeEntropy(img):
    ...
    for x in range (0, 256) :
        prob = 1.0 / s * np.count_nonzero(img == x)
...
def computeErrorImage(im1, im2):
    ...
    for x in range (0, h) :
      for y in range (0, w) :
          res[x,y] = int(im2[x,y]) - int(im1[x,y])

    return res
    
def computeDisplayableErrorImage(im1, im2):
    error = computeErrorImage(im1, im2)
    ...
    for x in range (0, h) :
      for y in range (0, w) :
          res[x,y] = min(255, max(0, error[x,y]+128))
...
def computeGME(flow):
    ...
    dst = np.ones_like(flow)
    dst = src + flow
    dstPts = dst.reshape((h*w, 2))
    
    res, mask = cv2.findHomography(srcPts, dstPts, cv2.RANSAC)
    
    default = np.zeros_like(flow)    
    gme = cv2.perspectiveTransform(default, res)
